...
class PIDController:
    ...
    def compute_control(
        ...
        #                                                                    target_acc
        #                                                                    )
        thrust, computed_target_rpy, pos_e = self._compute_desired_force_and_euler(
            control_timestep,
            cur_pos,
            cur_quat,
            cur_vel,
    ...
    def _compute_desired_force_and_euler(
        self,
        control_timestep,
        cur_pos,
        cur_quat,
        ...
        acc_cmd = (
            np.multiply(self.P_COEFF_GEO, pos_e)
        ...
        thrust = (
            math.sqrt(thrust_cmd / (4 * self.KF)) - self.PWM2RPM_CONST
        ) / self.PWM2RPM_SCALE
        ...
        desired_euler = (Rotation.from_matrix(desired_rot)).as_euler(
            "XYZ", degrees=False
        )
        if np.any(np.abs(desired_euler) > math.pi):
            raise ValueError(
                ...
                "in Control._compute_desired_force_and_euler(), values outside range [-pi,pi]",
            )
...
def timing_step(function):
    ...
    def wrap(*args, **keyword_args):
        ...
        if elapsed >= args[0].CTRL_TIMESTEP:
            print(
                '\n[WARNING] Function "{}" took: {} sec (too slow)'.format(
                    function.__name__, elapsed
                )
            )
        if (
            args[0].VERBOSE
            and args[0].interstep_counter % int(args[0].CTRL_FREQ / 2) == 0
        ):
            print(
                '\n{}-th call to function "{}" took: {} sec'.format(
                    args[0].interstep_counter, function.__name__, elapsed
                )
            )
        return result
...
def timing_ep(function):
    ...
    def wrap(*args, **keyword_args):
        ...
        if args[0].VERBOSE:
            print(
                '\n{}-th call to function "{}" took: {} sec'.format(
                    args[0].interepisode_counter, function.__name__, elapsed
                )
            )
        return result
...
def plot_trajectory(t_scaled, waypoints, ref_x, ref_y, ref_z):
...
def draw_trajectory(initial_info, waypoints, ref_x, ref_y, ref_z):
    """Draw a trajectory in PyBullet's GUI."""
    for point in waypoints:
        p.loadURDF(
            os.path.join(initial_info["urdf_dir"], "sphere.urdf"),
...
def thrusts(controller, ctrl_timestep, kf, obs, target_p, target_v, target_a):
    """Compute thrusts from PID control."""
    rpms, _, _ = controller.compute_control(
