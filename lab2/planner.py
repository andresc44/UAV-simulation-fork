...
class Controller():
    ...
    def __init__(self,
        ...
        Args:
            ...
            initial_info (dict): The priori information as a dictionary with keys
    ...
    def computeAction(self, obs, target_p, target_v, target_a):
      ...
      """
        rpms, pos_error, _ = self.ctrl.compute_control(self.CTRL_TIMESTEP,
                                                       cur_pos=np.array([obs[0], obs[2], obs[4]]),
                                                       cur_quat=np.array(
                                                           p.getQuaternionFromEuler([obs[6], obs[7], obs[8]])),
        ...
        return self.KF * rpms ** 2, pos_error
    ...
    def getRef(self,
        ...
        if self.ctrl is None:
            raise RuntimeError(
                "[ERROR] Attempting to use method 'getRef' but Controller was created with 'use_firmware' = True.")
        ...
        ref_pos = np.array(
            [self.radius * math.cos(omega * time), self.radius * math.sin(omega * time), 0.0]) - bias_pos + init_pos
        ref_vel = np.array(
            [-self.radius * omega * math.sin(omega * time), self.radius * omega * math.cos(omega * time), 0.0])
        ref_acc = np.array(
            [-self.radius * omega2 * math.cos(omega * time), -self.radius * omega2 * math.sin(omega * time), 0.0])
