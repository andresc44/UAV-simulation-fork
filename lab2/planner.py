...
class Controller:
    ...
    def __init__(
        ...
        self.k_p = initial_info["k_p"]
        self.k_d = initial_info["k_d"]
    ...
    def draw_circle(self, initial_info, radius):
        ...
        self.draw_trajectory(
            initial_info, np.array(ref_x), np.array(ref_y), np.array(ref_z)
        )
    ...
    def getRef(self, time, obs, reward=None, done=None, info=None):
        ...
        ref_pos = (
            np.array(
                [
                    self.radius * math.cos(omega * time),
                    self.radius * math.sin(omega * time),
                    0.0,
                ]
            )
            - bias_pos
        ...
        ref_vel = np.array(
            [
                -self.radius * omega * math.sin(omega * time),
                self.radius * omega * math.cos(omega * time),
                0.0,
            ]
        )
        ref_acc = np.array(
            [
                -self.radius * omega2 * math.cos(omega * time),
                -self.radius * omega2 * math.sin(omega * time),
                0.0,
            ]
        )
