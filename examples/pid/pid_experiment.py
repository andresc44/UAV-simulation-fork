...
def run(gui=True, n_episodes=1, n_steps=None, save_data=False):
    ...
    if (
        config.task_config.task == "traj_tracking"
        and config.task_config.task_info.trajectory_type == "custom"
    ):
    ...
    if custom_trajectory:
        ...
        ITERATIONS = (
            int(config.task_config["episode_len_sec"] * config.task_config["ctrl_freq"])
            + 2
        )  # +2 for start and end of reference

        # Curve fitting with waypoints.
        waypoints = np.array(
            [(0, 0, 0), (0.2, 0.5, 0.5), (0.5, 0.1, 0.6), (1, 1, 1), (1.3, 1, 1.2)]
        )
        deg = 6
    ...
    if config.task_config.task == "traj_tracking" and gui is True:
        if config.task_config.quad_type == 2:
            ref_3D = np.hstack(
                [
                    ctrl.env.X_GOAL[:, [0]],
                    np.zeros(ctrl.env.X_GOAL[:, [0]].shape),
                    ctrl.env.X_GOAL[:, [2]],
                ]
            )
        else:
        ...
        for i in range(10, ctrl.env.X_GOAL.shape[0], 10):
            p.addUserDebugLine(
                lineFromXYZ=[ref_3D[i - 10, 0], ref_3D[i - 10, 1], ref_3D[i - 10, 2]],
        ...
        if custom_trajectory:
            for point in waypoints:
                p.loadURDF(
                    os.path.join(ctrl.env.URDF_DIR, "gate.urdf"),
    ...
    trajs_data, metrics = experiment.run_evaluation(
    ...
    if save_data:
        ...
        with open(
            f"./temp-data/{config.algo}_data_{config.task_config.task}.pkl", "wb"
        ) as file:
    ...
    for i in range(iterations):
        ...
        obs, reward, done, info, action = (
            trajs_data["obs"][0][i],
            trajs_data["reward"][0][i],
            trajs_data["done"][0][i],
            trajs_data["info"][0][i],
            trajs_data["action"][0][i],
        )
