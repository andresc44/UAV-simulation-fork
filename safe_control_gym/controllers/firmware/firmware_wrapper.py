...
class FirmwareWrapper(BaseController):
    ...
    def reset(self):
        ...
        for i in range(3):
            firm.lpf2pInit(
                self.acclpf[i], self.firmware_freq, self.GYRO_LPF_CUTOFF_FREQ
            )
            firm.lpf2pInit(
                self.gyrolpf[i], self.firmware_freq, self.ACCEL_LPF_CUTOFF_FREQ
            )
        ...
        self.full_state_cmd_override = (
            True  # When true, high level commander is not called
        )
        ...
        init_vel = np.array(
            [init_obs[1], init_obs[3], init_obs[5]]
        )  # global coord, m/s
    ...
    def step(self, sim_time, action):
        ...
        while self.tick / self.firmware_freq < sim_time + self.ctrl_dt:
            ...
            # Estimate rates
            cur_rotation_rates = (
                cur_rpy - self.prev_rpy
            ) / self.firmware_dt  # body coord, rad/s
            ...
            if self.STATE_DELAY:
                ...
                self.state_history = self.state_history[1:] + [
                    [cur_pos, cur_vel, cur_acc, cur_rpy * self.RAD_TO_DEG]
                ]
            else:
                self._update_state(
                    state_timestamp,
            ...
            new_action = (
                ...
                * (
                    self.PWM2RPM_SCALE
                    * np.clip(np.array(self.pwms), self.MIN_PWM, self.MAX_PWM)
                    + self.PWM2RPM_CONST
                )
                ** 2
            )
    ...
    def _update_sensorData(
        self, timestamp, acc_vals, gyro_vals, baro_vals=[1013.25, 25]
    ):
        """
        Axis3f acc;               // Gs
        Axis3f gyro;              // deg/s
    ...
    def _update_baro(self, baro, pressure, temperature):
        ...
        baro.asl = (
            ((1015.7 / baro.pressure) ** 0.1902630958 - 1) * (25 + 273.15)
        ) / 0.0065
    ...
    def _step_controller(self):
        if not (self.sensorData_set):
            print(
                "WARNING: sensorData has not been updated since last controller call."
            )
    ...
    def _powerDistribution(self, control_t):
        ...
        if self.QUAD_FORMATION_X:
            ...
            motor_pwms += [
                self._motorsGetPWM(
                    self._limitThrust(control_t.thrust - r + p + control_t.yaw)
                )
            ]
            motor_pwms += [
                self._motorsGetPWM(
            ...
            motor_pwms += [
                self._motorsGetPWM(
                    self._limitThrust(
                        control_t.thrust + control_t.pitch + control_t.yaw
                    )
                )
            ]
            motor_pwms += [
