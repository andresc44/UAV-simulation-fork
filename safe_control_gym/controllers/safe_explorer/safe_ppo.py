...
class SafeExplorerPPO(BaseController):
    ...
    def reset(self):
        ...
        if self.training:
            if self.pretraining:
                ...
                assert (
                    self.pretrained
                ), "Must provide a pre-trained model for adaptation."
            ...
            self.c = np.array(
                [inf["constraint_values"][: self.num_constraints] for inf in info["n"]]
            )
        else:
    ...
    def learn(self, env=None, **kwargs):
        ...
        while self.total_steps < final_step:
            ...
            if (
                ...
                and self.total_steps % (final_step // self.num_checkpoints) == 0
            ):
    ...
    def log_step(self, results):
        ...
        if self.pretraining:
            ...
            for i in range(self.safety_layer.num_constraints):
                ...
                if "eval" in results:
                    self.logger.add_scalars(
                        {name: results["eval"][name]},
                        step,
                        prefix="constraint_loss_eval",
                    )
        else:
            # Learning stats.
            self.logger.add_scalars(
    ...
    def collect_constraint_data(self, num_steps):
