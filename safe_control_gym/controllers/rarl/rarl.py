...
class RARL(BaseController):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.adversary = PPOAgent(
            self.adv_obs_space, self.adv_act_space, **shared_agent_args
        )
    ...
    def collect_rollouts(self, adversary=False):
        ...
        for _ in range(self.rollout_steps):
            ...
            for idx, inf in enumerate(info["n"]):
                ...
                if "TimeLimit.truncated" in inff and inff["TimeLimit.truncated"]:
                    ...
                    if adversary:
                        terminal_val = (
                            self.adversary.ac.critic(terminal_obs_tensor)
                            .squeeze()
                            .detach()
                            .numpy()
                        )
        ...
        if adversary:
            last_val = (
                self.adversary.ac.critic(torch.FloatTensor(obs).to(self.device))
                .detach()
                .numpy()
            )
        else:
