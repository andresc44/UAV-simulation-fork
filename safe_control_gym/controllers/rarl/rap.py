...
class RAP(BaseController):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.agent = PPOAgent(
            self.env.observation_space, self.env.action_space, **shared_agent_args
        )
        ...
        self.adversaries = [
            PPOAgent(self.adv_obs_space, self.adv_act_space, **shared_agent_args)
        ...
        results.update(
            {
                ...
                "adv_indices": [adv_idx for adv_idx, _ in rollout_splits],
            }
        )
        return results
    ...
    def log_step(self, results):
        ...
        # learning stats
        self.logger.add_scalars(
            {k: results[k] for k in ["policy_loss", "value_loss", "entropy_loss"]},
            step,
            prefix="loss",
        )
    ...
    def collect_rollouts(self):
        ...
        # sample adversaries
        adv_indices = sorted(
            np.random.randint(self.num_adversaries, size=self.rollout_batch_size)
        )
        ...
        for _ in range(self.rollout_steps):
            ...
            for idx, inf in enumerate(info["n"]):
                ...
                if "TimeLimit.truncated" in inff and inff["TimeLimit.truncated"]:
                    ...
                    terminal_val_adv = (
                        adversary.ac.critic(terminal_obs_tensor)
                        .squeeze()
                        .detach()
                        .numpy()
                    )
        ...
        for idx, obs_adv in zip(indices_groups, obs_groups):
            out = (
                self.adversaries[idx]
                .ac.critic(torch.FloatTensor(obs_adv).to(self.device))
                .detach()
                .numpy()
            )
