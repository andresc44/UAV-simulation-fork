    ...
    def __init__(self, train_x, train_y, likelihood, nx):
        ...
        self.covar_module = gpytorch.kernels.ScaleKernel(
            gpytorch.kernels.RBFKernel(
                batch_shape=torch.Size([self.n]), ard_num_dims=train_x.shape[1]
            ),
...
class GaussianProcessCollection:
    ...
    def init_with_hyperparam(self, train_inputs, train_targets, path_to_statedicts):
        ...
        for gp_ind, gp in enumerate(self.gp_list):
            path = os.path.join(
                path_to_statedicts, f"best_model_{self.target_mask[gp_ind]}.pth"
            )
            print("#########################################")
            print("#       Loading GP dimension {self.target_mask[gp_ind]}         #")
            print("#########################################")
            print(f"Path: {path}")
            gp.init_with_hyperparam(
                train_inputs, train_targets[:, self.target_mask[gp_ind]], path
            )
    ...
    def get_hyperparameters(self, as_numpy=False):
        ...
        if as_numpy:
            return (
                ...
                outputscale.numpy(),
                noise.numpy(),
                self.K_plus_noise.detach().numpy(),
            )
        else:
    ...
    def train(
        ...
        for gp_ind, gp in enumerate(self.gp_list):
            ...
            gp.train(
                ...
                gpu=gpu,
                fname=os.path.join(
                    output_dir, f"best_model_{self.target_mask[gp_ind]}.pth"
                ),
            )
    ...
    def make_casadi_predict_func(self):
        ...
        casadi_predict = ca.Function("pred", [z], [y], ["z"], ["mean"])
    ...
    def kernel_inv(self, x1, x2=None):
        ...
        non_lazy_tensors = [
            k.inv_matmul(torch.eye(num_of_points).double()) for k in k_list
        ]
        return torch.stack(non_lazy_tensors)
...
class GaussianProcess:
    ...
    def __init__(
        self, model_type, likelihood, input_mask=None, target_mask=None, normalize=False
    ):
    ...
    def train(
        ...
        while i < n_train and torch.abs(loss - last_loss) > 1e-2:
            ...
            if i % 100 == 0:
                print(
                    "Iter %d/%d - MLL trian Loss: %.3f, Posterior Test Loss: %0.3f"
    ...
    def make_casadi_prediction_func(self, train_inputs, train_targets):
        ...
        predict = ca.Function(
            ...
            [
                K_z_ztrain(z=z)["K"]
                @ self.model.K_plus_noise_inv.detach().numpy()
                @ train_targets
            ],
            ["z"],
            ["mean"],
    ...
    def plot_trained_gp(self, inputs, targets, output_label, fig_count=0):
        ...
        for i in range(self.output_dimension):
            ...
            if lower.ndim > 1:
                plt.fill_between(
                    t,
                    lower[:, i].detach().numpy(),
                    upper[:, i].detach().numpy(),
                    alpha=0.5,
                    label="95%",
                )
