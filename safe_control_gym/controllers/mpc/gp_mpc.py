...
class GPMPC(MPC):
    ...
    def create_sparse_GP_machinery(self, n_ind_points):
        ...
        lengthscales, signal_var, noise_var, gp_K_plus_noise = (
    ...
    def preprocess_training_data(self, x_seq, u_seq, x_next_seq):
        ...
        x_pred_seq = self.prior_dynamics_func(
            x0=x_seq.T - self.prior_ctrl.X_EQ[:, None],
            p=u_seq.T - self.prior_ctrl.U_EQ[:, None],
        )["xf"].toarray()
        targets = (
            x_next_seq.T - (x_pred_seq + self.prior_ctrl.X_EQ[:, None])
        ).transpose()  # (N, nx).
    ...
    def precompute_probabilistic_limits(self, print_sets=False):
        ...
        for state_constraint in self.constraints.state_constraints:
            state_constraint_set.append(
                np.zeros((state_constraint.num_constraints, T + 1))
            )
            ...
            for i in range(T):
                ...
                if self.gp_approx == "taylor":
                    raise NotImplementedError(
                        "Taylor GP approximation is currently not working."
                    )
                elif self.gp_approx == "mean_eq":
                    _, cov_d_tensor = self.gaussian_process.predict(
                        z[None, :], return_pred=False
                    )
                    ...
                    raise NotImplementedError(
                        "gp_approx method is incorrect or not implemented"
                    )
                # Loop through input constraints and tighten by the required ammount.
                for ui, input_constraint in enumerate(
                ...
                for si, state_constraint in enumerate(
                    self.constraints.state_constraints
                ):
    ...
    def setup_gp_optimizer(self, n_ind_points):
        ...
        if self.gp_soft_constraints:
            state_slack_list = []
            for state_constraint in self.constraints.state_constraints:
                state_slack_list.append(
                    opti.variable(state_constraint.num_constraints, T + 1)
                )
            input_slack_list = []
            for input_constraint in self.constraints.input_constraints:
                input_slack_list.append(
                    opti.variable(input_constraint.num_constraints, T)
                )
        ...
        for state_constraint in self.constraints.state_constraints:
            state_constraint_set.append(
                opti.parameter(state_constraint.num_constraints, T + 1)
            )
        input_constraint_set = []
        for input_constraint in self.constraints.input_constraints:
            input_constraint_set.append(
                opti.parameter(input_constraint.num_constraints, T)
            )
        ...
        for i in range(self.T):
            ...
            if True and self.sparse_gp:
                next_state = (
                    self.prior_dynamics_func(
                        x0=x_var[:, i] - self.prior_ctrl.X_EQ[:, None],
                        p=u_var[:, i] - self.prior_ctrl.U_EQ[:, None],
                    )["xf"]
                    + self.prior_ctrl.X_EQ[:, None]
                    + self.Bd
                    @ cs.sum2(
                        self.K_z_zind_func(z1=z[:, i].T, z2=z_ind)["K"]
                        * mean_post_factor
                    )
                )
            else:
            ...
            for s_i, state_constraint in enumerate(self.state_constraints_sym):
                if self.gp_soft_constraints:
                    opti.subject_to(
                        state_constraint(x_var[:, i])
                        <= state_constraint_set[s_i][:, i] + state_slack_list[s_i][:, i]
                    )
                    cost += (
                        soft_con_coeff
                        * state_slack_list[s_i][:, i].T
                        @ state_slack_list[s_i][:, i]
                    )
                    opti.subject_to(state_slack_list[s_i][:, i] >= 0)
                else:
                    opti.subject_to(
                        state_constraint(x_var[:, i])
                        <= state_constraint_set[s_i][:, i] - self.constraint_tol
                    )
            for u_i, input_constraint in enumerate(self.input_constraints_sym):
                if self.gp_soft_constraints:
                    opti.subject_to(
                        input_constraint(u_var[:, i])
                        <= input_constraint_set[u_i][:, i] + input_slack_list[u_i][:, i]
                    )
                    cost += (
                        soft_con_coeff
                        * input_slack_list[u_i][:, i].T
                        @ input_slack_list[u_i][:, i]
                    )
                    opti.subject_to(input_slack_list[u_i][:, i] >= 0)
                else:
                    opti.subject_to(
                        input_constraint(u_var[:, i])
                        <= input_constraint_set[u_i][:, i] - self.constraint_tol
                    )
        ...
        for s_i, state_constraint in enumerate(self.state_constraints_sym):
            if self.gp_soft_constraints:
                opti.subject_to(
                    state_constraint(x_var[:, -1])
                    <= state_constraint_set[s_i][:, -1] + state_slack_list[s_i][:, -1]
                )
                cost += (
                    soft_con_coeff
                    * state_slack_list[s_i][:, -1].T
                    @ state_slack_list[s_i][:, -1]
                )
                opti.subject_to(state_slack_list[s_i][:, -1] >= 0)
            else:
                opti.subject_to(
                    state_constraint(x_var[:, -1])
                    <= state_constraint_set[s_i][:, -1] - self.constraint_tol
                )

        # Bound constraints.
        upper_state_bounds = np.clip(
            self.prior_ctrl.env.observation_space.high, None, 10
        )
        lower_state_bounds = np.clip(
            self.prior_ctrl.env.observation_space.low, -10, None
        )
        ...
        for i in range(self.T):
            opti.subject_to(
                opti.bounded(
                    lower_state_bounds + self.constraint_tol,
                    x_var[:, i],
            ...
            opti.subject_to(
                opti.bounded(
                    lower_input_bounds + self.constraint_tol,
                    u_var[:, i],
    ...
    def select_action_with_gp(self, obs):
        ...
        if self.warmstart and self.x_prev is None and self.u_prev is None:
            x_guess, u_guess = self.prior_ctrl.compute_initial_guess(
                obs, goal_states, self.X_EQ, self.U_EQ
            )
        ...
        gp_contribution = np.sum(
            self.K_z_zind_func(z1=zi, z2=z_ind_val)["K"].toarray()
        ...
        lin_pred = (
            self.prior_dynamics_func(
                x0=x_val[:, 0] - self.prior_ctrl.X_EQ,
                p=u_val[:, 0] - self.prior_ctrl.U_EQ,
            )["xf"].toarray()
            + self.prior_ctrl.X_EQ[:, None]
        )
    ...
    def reset(self):
        ...
        if self.gaussian_process is not None:
            if (
                self.sparse_gp
                and self.train_data["train_targets"].shape[0] <= self.n_ind_points
            ):
                n_ind_points = self.train_data["train_targets"].shape[0]
            elif self.sparse_gp:
                n_ind_points = self.n_ind_points
