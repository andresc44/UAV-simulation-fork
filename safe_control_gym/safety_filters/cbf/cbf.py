...
class CBF(BaseSafetyFilter):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.input_constraints_sym = (
            self.env.constraints.get_input_constraint_symbolic_models()
        )
        self.state_constraints_sym = (
            self.env.constraints.get_state_constraint_symbolic_models()
        )
        ...
        if len(input_constraints) > 1:
            raise NotImplementedError(
                "CBF currently can't handle more than 1 constraint"
            )
        ...
        if self.env.NAME == "cartpole":
            self.state_limits = [
                min(
                    abs(self.state_constraint.upper_bounds[i]),
                    abs(self.state_constraint.lower_bounds[i]),
                )
            ...
            raise NotImplementedError(
                "[Error] Currently CBF is only implemented for the cartpole system."
            )
    ...
    def setup_optimizer(self):
        ...
        # CBF constraint
        opti.subject_to(
            -self.linear_func(x=barrier_at_x)["y"] - lie_derivative_at_x
            <= right_hand_side
        )
    ...
    def certify_action(
        ...
        uncertified_action = np.clip(
            uncertified_action,
            self.env.physical_action_bounds[0],
            self.env.physical_action_bounds[1],
        )
        self.results_dict["uncertified_action"].append(uncertified_action)
        certified_action, success = self.solve_optimization(
            current_state, uncertified_action
        )
        ...
        self.results_dict["correction"].append(
            np.linalg.norm(certified_action - uncertified_action)
        )

        return certified_action, success

    def is_cbf(
        self, num_points: int = 100, tolerance: float = 0.01
    ) -> Tuple[bool, list]:
        """
        ...
        states_to_sample = [
            np.linspace(min_bounds[i], max_bounds[i], num_points_per_dim)
            for i in range(nx)
        ]
        ...
        if num_infeasible_states_inside_set > 0:
            ...
            valid_cbf = True
            print(
                "The provided CBF candidate is a valid CBF inside its superlevel set for the checked states. "
            ...
            valid_cbf = True
            print(
                "The provided CBF candidate is a valid CBF for the checked states. "
                "Consider increasing the sampling resolution to get a more precise evaluation."
            )
