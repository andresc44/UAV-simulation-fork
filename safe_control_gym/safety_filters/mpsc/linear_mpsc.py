...
class LINEAR_MPSC(MPSC):
    ...
    def set_dynamics(self):
        ...
        if self.integration_algo == "LTI":
            ...
            dynamics_func = cs.Function(
                "fd", [delta_x, delta_u], [x_dot_lin_vec], ["x0", "p"], ["xf"]
            )
        elif self.integration_algo == "rk4":
            dynamics_func = rk_discrete(
            ...
            dynamics_func = cs.integrator(
                ...
                {"x": delta_x, "p": delta_u, "ode": x_dot_lin_vec},
                {"tf": self.dt},
            )
    ...
    def learn(self, env=None):
        ...
        for i in range(self.n_samples):
            ...
            x_next_linear = (
                np.squeeze(
                    self.dynamics_func(x0=init_state - self.X_EQ, p=u - self.U_EQ)[
                        "xf"
                    ].toarray()
                )
                + self.X_EQ
        ...
        self.omega_constraint = QuadraticContstraint(
            self.env, self.P, 1.0, constrained_variable=ConstrainedVariableType.STATE
        )
        ...
        if self.learn_terminal_set:
            ...
            for i in range(self.n_samples_terminal_set):
                if self.terminal_set is None:
                    ...
                    init_state = self.terminal_set.V[
                        np.random.choice(self.terminal_set.V.shape[0], 1)
                    ]

                init_state = init_state.reshape((self.model.nx, 1))
                init_state += (
                    np.random.rand(self.model.nx, 1) - np.ones((self.model.nx, 1)) / 2
                ) / 2
                ...
                _, feasible = self.solve_optimization(
                    obs=init_state, uncertified_action=u
                )
                if feasible:
                    if self.terminal_set is None:
    ...
    def tighten_state_and_input_constraints(self):
        ...
        K_omega_AABB_verts_raw_limits = np.array(
            [
                np.amax(K_omega_AABB_verts_raw, axis=0),
                np.amin(K_omega_AABB_verts_raw, axis=0),
            ]
        )
        self.K_omega_AABB_verts = np.vstack(
            list(product(*(K_omega_AABB_verts_raw_limits.T)))
        )
        ...
        if len(input_constraint) > 1:
            raise NotImplementedError(
                "MPSC currently can't handle more than 1 constraint"
            )

        input_constraint = input_constraint[0]
        if self.training_env.NAME != Environment.QUADROTOR:
            U_vertices_raw = [
                (input_constraint.upper_bounds[i], input_constraint.lower_bounds[i])
                for i in range(self.model.nu)
            ]
        else:
            U_vertices_raw = [
                (input_constraint.upper_bounds[i], -input_constraint.upper_bounds[i])
                for i in range(self.model.nu)
            ]
        ...
        self.tightened_input_constraint_verts, tightened_input_constr_func = (
            pontryagin_difference_AABB(self.U_vertices, self.K_omega_AABB_verts)
        )

        if self.training_env.NAME == Environment.QUADROTOR:
            min_input = (
                input_constraint.lower_bounds[0]
                + np.max(self.U_vertices)
                - np.max(self.tightened_input_constraint_verts)
            )
            self.tightened_input_constraint_verts = np.clip(
                self.tightened_input_constraint_verts, min_input, 100
            )
        self.tightened_input_constraint = tightened_input_constr_func(
            env=self.env, constrained_variable=ConstrainedVariableType.INPUT
        )
        ...
        X_vertices_raw = [
            (state_constraints.upper_bounds[i], state_constraints.lower_bounds[i])
            for i in range(self.model.nx)
        ]
        self.X_vertices = np.clip(np.vstack(list(product(*X_vertices_raw))), -100, 100)
        self.tightened_state_constraint_verts, tightened_state_constraint_func = (
            pontryagin_difference_AABB(self.X_vertices, self.omega_AABB_verts)
        )
        self.tightened_state_constraint = tightened_state_constraint_func(
            env=self.env, constrained_variable=ConstrainedVariableType.STATE
        )

        self.simple_terminal_set = QuadraticContstraint(
            env=self.env,
    ...
    def setup_optimizer(self):
        ...
        if self.use_terminal_set:
            if self.terminal_set is not None:
                terminal_constraint = LinearConstraint(
                    env=self.env,
                    A=self.terminal_set.A,
                    b=self.terminal_set.b,
                    constrained_variable=ConstrainedVariableType.STATE,
                )
        ...
        # Real input (5.f).
        opti.subject_to(
            next_u == (v_var[:, 0] + self.U_EQ) + self.lqr_gain @ (x_init - z_var[:, 0])
        )
