...
class MPSC(BaseSafetyFilter, ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.set_dynamics()
        self.lqr_gain = -compute_lqr_gain(
            self.model, self.X_EQ, self.U_EQ, self.Q, self.R, discrete_dynamics=True
        )
        ...
        self.constraints, self.state_constraints_sym, self.input_constraints_sym = (
            reset_constraints(self.env.constraints.constraints + additional_constraints)
        )
        ...
        if cost_function == Cost_Function.ONE_STEP_COST:
            ...
            raise NotImplementedError(
                f"The MPSC cost function {cost_function} has not been implemented"
            )
    ...
    def certify_action(
        ...
        action, feasible = self.solve_optimization(
            current_state, uncertified_action, iteration
        )
        ...
        if feasible:
            ...
            if (
                ...
                and self.z_prev is not None
                and self.v_prev is not None
            ):
                action = np.squeeze(self.v_prev[:, self.kinf]) + np.squeeze(
                    self.lqr_gain
                    @ (
                        current_state.reshape((self.model.nx, 1))
                        - self.z_prev[:, self.kinf].reshape((self.model.nx, 1))
                    )
                )
