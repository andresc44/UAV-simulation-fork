...
def compute_RPI_set(Acl, w, tau):
    ...
    for i in range(n_samples):
        ...
        constraints += [cp.bmat([[con_11, con_12], [con_21, con_22]]) << 0]
    ...
    try:
        ...
        print(
            "[ERROR] RPI Computation failed. Ensure you have the MOSEK solver. Otherwise, error unknown."
        )
        exit()
    return P.value
...
def pontryagin_difference_AABB(verts1, verts2):
    ...
    if verts1.shape[1] > 1:
        ...
        const_func = partial(
            BoundedConstraint, lower_bounds=vert_min, upper_bounds=vert_max
        )
...
def get_trajectory_on_horizon(env, iteration, horizon):
    ...
    if env.TASK == Task.TRAJ_TRACKING:
        wp_idx = [min(iteration + i, env.X_GOAL.shape[0] - 1) for i in range(horizon)]
