...
class Quadrotor(BaseAviary):
    ...
    INERTIAL_PROP_RAND_INFO = {
        ...
        "Ixx": {"distrib": "uniform", "low": 1.3e-5, "high": 1.5e-5},  # Nominal: 1.4e-5
    ...
    def __init__(
        ...
        **kwargs
        ...
        if info_mse_metric_state_weight is None:
            ...
            if (
                (
                    self.QUAD_TYPE == QuadType.ONE_D
                    and len(info_mse_metric_state_weight) == 2
                )
                or (
                    self.QUAD_TYPE == QuadType.TWO_D
                    and len(info_mse_metric_state_weight) == 6
                )
                or (
                    self.QUAD_TYPE == QuadType.THREE_D
                    and len(info_mse_metric_state_weight) == 12
                )
            ):
                self.info_mse_metric_state_weight = np.array(
        ...
        elif self.QUAD_TYPE == QuadType.THREE_D and np.array(inertial_prop).shape == (
            4,
        ):
        ...
        if (
            "randomized_gates_and_obstacles" in kwargs
            and kwargs["randomized_gates_and_obstacles"]
        ):
            self.RANDOMIZED_GATES_AND_OBS = True
            if "gates_and_obstacles_randomization_info" not in kwargs:
                raise ValueError(
                    "[ERROR] Missing 'gates_and_obstacles_randomization_info' in YAML configuration."
                )
            self.GATES_AND_OBS_RAND_INFO = kwargs[
    ...
    def reset(self):
        ...
        if self.RANDOMIZED_GATES_AND_OBS:
            ...
            distrib = getattr(
                self.np_random, rand_info_copy["obstacles"].pop("distrib")
            )
        ...
        for gate in self.GATES:
            if gate[6] == 0:
                ...
                gate_height = (
                    0.525  # URDF dependent, places 'low_portal.urdf' at z == 0.
                )
            else:
            ...
            self.EFFECTIVE_GATES_POSITIONS.append(
                list(np.array(gate[0:3]) + offset)
                + list(np.array(gate[3:6]) + pose_disturbance)
            )
            TMP_ID = p.loadURDF(
    ...
    def _setup_symbolic(self):
        ...
        if self.QUAD_TYPE == QuadType.ONE_D:
            ...
            oVdot_cg_o = Rob @ cs.vertcat(0, 0, f1 + f2 + f3 + f4) / m - cs.vertcat(
                0, 0, g
            )
            pos_ddot = oVdot_cg_o
            ...
            rate_dot = Jinv @ (
                Mb - (cs.skew(cs.vertcat(p, q, r)) @ J @ cs.vertcat(p, q, r))
            )
            ang_dot = cs.blockcat(
                [
    ...
    def _set_action_space(self):
        ...
        if self.NORMALIZED_RL_ACTION_SPACE:
            ...
            a_low = (
                self.KF
                * n_motors
                * (self.PWM2RPM_SCALE * self.MIN_PWM + self.PWM2RPM_CONST) ** 2
            )
            a_high = (
                self.KF
                * n_motors
                * (self.PWM2RPM_SCALE * self.MAX_PWM + self.PWM2RPM_CONST) ** 2
            )
            self.action_space = spaces.Box(
    ...
    def _get_info(self):
        ...
        for GATE_OBS_ID in (
            self.GATES_IDS + self.OBSTACLES_IDS
        ):  # remove the ground from object list to avoid collision
        ...
        if self.current_gate < self.NUM_GATES:
            ...
            closest_points = p.getClosestPoints(
                bodyA=self.GATES_IDS[self.current_gate],
                bodyB=self.DRONE_IDS[0],
            ...
            if len(closest_points) > 0:
                ...
                info["current_target_gate_pos"] = self.EFFECTIVE_GATES_POSITIONS[
                    self.current_gate
                ]
            else:
        ...
        if self.current_gate == self.NUM_GATES:
            if self.QUAD_TYPE == QuadType.THREE_D:
                ...
                print(
                    '[WARNING] "at_goal_position" and "task_completed" are only intended for used with the 3D quadrotor.'
                )
