...
class SubprocVecEnv(VecEnv):
    ...
    def __init__(self, env_fns, spaces=None, context="spawn", n_workers=1):
        ...
        assert (
            nenvs % n_workers == 0
        ), "Number of envs must be divisible by number of workers to run in series"
    ...
    def set_attr(self, attr_name, values, indices=None):
        ...
        for remote, env_indices, value_split in zip(
            target_remotes, remote_env_indices, value_splits
        ):
    ...
    def env_method(
        self, method_name, method_args=None, method_kwargs=None, indices=None
    ):
        ...
        for remote, env_indices, method_arg_split, method_kwarg_split in zip(
            target_remotes, remote_env_indices, method_arg_splits, method_kwarg_splits
        ):
            remote.send(
                (
                    "env_method",
                    (env_indices, method_name, method_arg_split, method_kwarg_split),
                )
            )
        return _flatten_list([remote.recv() for remote in target_remotes])
    ...
    def _get_target_envs(self, indices):
        ...
        assert indices is None or sorted(indices) == indices, "Indices must be ordered"
        ...
        while True:
            ...
            if cmd == "step":
                ...
                remote.send(
                    [
                        method(*args, **kwargs)
                        for method, args, kwargs in zip(methods, args_list, kwargs_list)
                    ]
                )
            else:
