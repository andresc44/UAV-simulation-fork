...
class CartPole(BenchmarkEnv):
    ...
    URDF_PATH = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "assets", "cartpole_template.urdf"
    )

    AVAILABLE_CONSTRAINTS = {"abs_bound": SymmetricStateConstraint}
    ...
    INERTIAL_PROP_RAND_INFO = {
        "pole_length": {"distrib": "choice", "args": [[1, 5, 10]]},  # Nominal: 1
        "cart_mass": {"distrib": "uniform", "low": 0.5, "high": 1.5},  # Nominal: 1
        "pole_mass": {"distrib": "uniform", "low": 0.05, "high": 0.15},  # Nominal: 0.1
    }

    INIT_STATE_RAND_INFO = {
        "init_x": {"distrib": "uniform", "low": -0.05, "high": 0.05},
        "init_x_dot": {"distrib": "uniform", "low": -0.05, "high": 0.05},
        "init_theta": {"distrib": "uniform", "low": -0.05, "high": 0.05},
        "init_theta_dot": {"distrib": "uniform", "low": -0.05, "high": 0.05},
    }
        ...
        **kwargs,
        ...
        if self.TASK == Task.STABILIZATION:
            self.X_GOAL = np.hstack(
                [self.TASK_INFO["stabilization_goal"][0], 0.0, 0.0, 0.0]
            )  # x = {x, x_dot, theta, theta_dot}.
        elif self.TASK == Task.TRAJ_TRACKING:
            POS_REF, VEL_REF, _ = self._generate_trajectory(
    ...
    def step(self, action):
        ...
        self.state = np.hstack(
            (
                p.getJointState(
                    self.CARTPOLE_ID, jointIndex=0, physicsClientId=self.PYB_CLIENT
                )[0:2],
                p.getJointState(
                    self.CARTPOLE_ID, jointIndex=1, physicsClientId=self.PYB_CLIENT
                )[0:2],
            )
        )
        # Standard Gym return.
    ...
    def reset(self, seed=None):
        ...
        p.setAdditionalSearchPath(
            pybullet_data.getDataPath(), physicsClientId=self.PYB_CLIENT
        )
        ...
        self.CARTPOLE_ID = p.loadURDF(
            self.override_path, basePosition=[0, 0, 0], physicsClientId=self.PYB_CLIENT
        )
    ...
    def _setup_symbolic(self, prior_prop={}, **kwargs):
        ...
        theta_dot_dot = (g * cs.sin(theta) - cs.cos(theta) * temp_factor) / (
            length * (4.0 / 3.0 - m * cs.cos(theta) ** 2 / Mm)
        )
        X_dot = cs.vertcat(
            x_dot,
            temp_factor - ml * theta_dot_dot * cs.cos(theta) / Mm,
            theta_dot,
            theta_dot_dot,
        )
        # Observation.
        ...
        self.symbolic = SymbolicModel(
            dynamics=dynamics, cost=cost, dt=dt, params=params
        )
    ...
    def _set_action_space(self):
        ...
        self.physical_action_bounds = (
            -1 * np.atleast_1d(self.action_scale),
            np.atleast_1d(self.action_scale),
        )
        self.action_threshold = (
            1 if self.NORMALIZED_RL_ACTION_SPACE else self.action_scale
        )
        self.action_space = spaces.Box(
            low=-self.action_threshold, high=self.action_threshold, shape=(1,)
        )
    ...
    def _set_observation_space(self):
        ...
        self.observation_space = spaces.Box(
            low=-obs_bound, high=obs_bound, dtype=np.float32
        )
    ...
    def _preprocess_control(self, action):
        ...
        force = np.clip(
            action, self.physical_action_bounds[0], self.physical_action_bounds[1]
        )
    ...
    def _get_observation(self):
        ...
        if (
            not np.array_equal(
                self.state,
                np.clip(self.state, self.state_space.low, self.state_space.high),
            )
            and self.VERBOSE
        ):
    ...
    def _get_reset_info(self):
        ...
        if self.constraints is not None:
            ...
            )  # Fix for input constraints only
        return info
    ...
    def _parse_urdf_parameters(self, file_name):
        ...
        EFFECTIVE_POLE_LENGTH = 0.5 * float(
            URDF_TREE[3][0][0][0].attrib["size"].split(" ")[-1]
