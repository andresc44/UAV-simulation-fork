...
class GeoController:
    ...
    def compute_control(
        ...
        desired_thrust, desire_rpy, pos_e = self._compute_desired_force_and_euler(
            control_timestep,
            cur_pos,
            cur_quat,
            cur_vel,
        ...
        rpm = self._compute_rpms(
            control_timestep, desired_thrust, cur_quat, desire_rpy, target_rpy_rates
        )
    ...
    def _compute_desired_force_and_euler(
        ...
        cur_y_b: np.ndarray = np.array(
            [-np.sin(reference_yaw), np.cos(reference_yaw), 0]
        )
        desired_z_b: np.ndarray = desired_acc
        desired_x_b = np.cross(cur_y_b, desired_z_b)
        desired_y_b = np.cross(desired_z_b, desired_x_b)
        desired_rotation = np.vstack(
            (
                desired_x_b / np.linalg.norm(desired_x_b),
                desired_y_b / np.linalg.norm(desired_y_b),
                desired_z_b / np.linalg.norm(desired_z_b),
            )
        ).T
    ...
    def _compute_rpms(
        ...
        thrust = (
            math.sqrt(thrust / (4 * self.KF)) - self.PWM2RPM_CONST
        ) / self.PWM2RPM_SCALE

        self.P_COEF_TOR = np.array([70000.0, 70000.0, 60000.0])
        self.I_COEF_TOR = np.array([0.0, 0.0, 500.0])
        self.D_COEF_TOR = np.array([20000.0, 20000.0, 12000.0])
        ...
        # PID target torques.
        target_torques = (
            -np.multiply(self.P_COEF_TOR, rot_e)
