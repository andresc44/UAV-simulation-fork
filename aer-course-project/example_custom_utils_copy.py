...
ORDER = [4, 1,4] # Gate Order, Slightly slower for 4, 3, 2, 1
# ORDER = [4]
DRONE_SPEED = 1.0 #m/s
...
GATE_STRAIGHT_WEIGHT = 40 #How vital is it to go more straight during gate
...
def shortest_curved_path(points, time_offset, dt, obs_bounds, last_segment):
    ...
    t1 = straight_segment_weight/1
    if last_segment: t2 = len(full_traj)-1
    else: t2 = len(full_traj)-1-(straight_segment_weight/1)
    ...
    # Integrate the velocity magnitude function over the time interval
    total_distance, _ = quad(velocity_magnitude, t1, t2)
    total_distance, _ = quad(velocity_magnitude, 0, len(full_traj))
    

    print("Distance along the polygonal path:", total_distance, "t2: ", len(full_traj)-1)
    if last_segment:
        print("go fast for last")
        duration = total_distance / (DRONE_SPEED*1)
    elif points[4,0]==2.0:
        print("go fast for gate 2")
        duration = total_distance / (DRONE_SPEED*1)
    else:
    ...
    t = np.linspace(0, len(full_traj), intervals+1)
    ...
    if not last_segment:
        temptraj=np.linspace(points[3], points[5], 30)
        temp_x=temptraj[:,0]
        # print(temp_x.shape)
        temp_y=temptraj[:,1]
        temp_time=np.arange(0, len(temp_x) * dt, dt)
        # print(temp_time.shape)
        temp_offset=last_time+dt
        temp_time=np.add(temp_time, temp_offset)
        # print(temp_time.shape)

        fitted_x=np.append(fitted_x,temp_x, axis=0)
        fitted_y=np.append(fitted_y,temp_y, axis=0)
        time=np.append(time,temp_time, axis=0)
        # print(fitted_x.shape)
